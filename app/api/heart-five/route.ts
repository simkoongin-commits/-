    ...
    const records = recordsSnapshot.docs
      ...
      .map((record) => {
        ...
        return {
          ...
          stats: unlocked ? recordStats(record.shots) : null,
        };
      });
    return NextResponse.json({ records, points: walletPoints(walletSnapshot.data()?.points) }, { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
