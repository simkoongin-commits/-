...
- `lib/members.ts`: 회원 타입과 학기별 등급, 직책별 고정 팀 규칙의 단일 기준입니다.
...
- 기존 `clubs/heartFive-{uuid}`와 `clubs/simgunghoe/shotRecords` 기록은 첫 조회 시 서버가 `heartFiveRecords`로 옮기고, 완성된 기록마다 소유자에게 2점을 한 번 지급합니다. 새 기록은 한 순 이상을 완성하고 `저장`할 때 2점이 한 번만 지급되며, 기록을 삭제하면 2점을 회수합니다. 이미 사용한 경우 잔액이 음수가 될 수 있고 이후 획득 점수로 갚습니다.
...
- 기록 입력 화면의 `수정`을 누르면 기존 시를 바꾸거나 마지막 시를 삭제할 수 있습니다. 입력 중인 기록은 자동으로 임시저장되고, 완성된 순은 `저장`을 눌러 목록과 통계에 공개합니다.
